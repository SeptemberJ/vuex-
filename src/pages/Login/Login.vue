<template>
	<div class="LoginWrap" style="">
    	...
    	<div style="position:absolute;top:100px;left:50%;width:50%;height:300px;">
	    	<h1 class="marginTB_20">SmartHox Data analysis platform</h1>
	    	<Form class="marginT_20" ref="formLogin" :model="formLogin" :rules="ruleLogin" style="width:200px;height:200px;margin-top:50px;">
	            <Row>
	              <Col span="24">
	                <FormItem prop="phone">
	                ...
	                <FormItem>
	                    <Button type="default" :style="{width:'100%'}" :loading="loading" @click="handleSubmit('formLogin')">
...
export default {
	...
	methods: {
		...mapActions([
			'ToggleSpin', // 映射 this.increment() 为 this.$store.dispatch('increment')
		]),
		//...mapActions({
		...
		drawLine(){
			...
			maskImage.src = 'static/img/cat.jpg';
			var keywords = {
	            "visualMap": 22199,
	            "continuous": 10288,
	            "contoller": 620,
		...
		handleSubmit(name){
		    ...
		    axios.post('http://222.44.17.8:8085/znkzJK/login',DATA
			).then((res)=> {
			}).catch((error)=> {
			console.log(error)
			})
			/*
			this.$http.post('http://222.44.17.8:8085/znkzJK/login', DATA)
		    .then((response) => {
